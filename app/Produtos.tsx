...
import { listarProdutos, cadastrarProduto, excluirProduto } from '../src/storage/produtosStorage';
import { MaterialCommunityIcons } from '@expo/vector-icons';
import * as Crypto from 'expo-crypto';
import { useSafeAreaInsets } from 'react-native-safe-area-context';

// Componente para a Lista de Produtos
const ListaProdutosView = ({ produtos, onRefresh, refreshing, onEdit, onDelete, onAddNew }) => {
...
const FormularioProduto = ({ produtoEditando, onSave, onCancel, isSaving }) => {
...
export default function ProdutosScreen() {
    ...
    const handleSalvarProduto = async ({ descricao, valor }) => {
        const valorNum = parseFloat(valor.replace(',', '.')) || 0;
        if (!descricao.trim()) {
            Alert.alert('Atenção', 'Nome do produto é obrigatórios.');
            return;
        }
        setIsSaving(true);
        try {
...
const styles = StyleSheet.create({
    ...
    formContainer: { flex: 1, paddingHorizontal: 16, paddingVertical: 10 },
