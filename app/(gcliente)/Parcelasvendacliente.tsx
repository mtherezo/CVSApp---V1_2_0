...
const MemoizedPagamentoItem = React.memo(({ item, onExcluir, isDeleting, isSubmitting }: { item: Pagamento, onExcluir: (item: Pagamento) => void, isDeleting: boolean, isSubmitting: boolean }) => (
    <View style={styles.itemPagamento}>
        <View style={styles.itemPagamentoInfo}>
            ...
            <Text style={styles.itemData}>Pago em: {new Date(item.dataPagamento).toLocaleDateString('pt-BR')}</Text>
        </View>
        <TouchableOpacity
...
const FormularioNovoPagamento = ({ saldoDevedor, onRegistrarPagamento, isSubmitting }: {
    saldoDevedor: number;
    onRegistrarPagamento: (valor: string, data: Date) => void;
    ...
    const handlePress = () => {
        ...
        onRegistrarPagamento(valorPagamento, dataPagamento);
        setValorPagamento('');
    ...
    return (
        <View style={styles.novoPagamentoContainer}>
            ...
            <View style={styles.inputRow}>
                <TouchableOpacity style={styles.datePickerButton} onPress={() => setShowDatePicker(true)} disabled={isSubmitting}>
                    <MaterialCommunityIcons name="calendar" size={22} color="#A9A9A9" />
                    <Text style={styles.datePickerText}>{dataPagamento.toLocaleDateString('pt-BR')}</Text>
                </TouchableOpacity>
...
export default function ParcelasVendaClienteScreen() {
    ...
    const handleRegistrarPagamento = useCallback(async (valorString: string, dataSelecionada: Date) => {
        ...
        try {
            await registrarPagamentoSQLite(idVenda, valor, dataSelecionada.toISOString());
