...
export default function UpgradeScreen() {
  const { colors } = useTheme(); // <-- CORRIGIDO
  const styles = createStyles(colors); // <-- CORRIGIDO
  const { unlockPremium } = usePremium(); // Pegamos a função para atualizar o status
  ...
  const handleCompraSimulada = () => {
    ...
    setTimeout(async () => {
      ...
      // 2. Avisa o usuário (Idealmente, usar um modal/toast) - CORRIGIDO: use unlockPremium
...
function setPremiumStatus(arg0: boolean) {
